...
export const DataButton = memo(({
  ...
  return (
    <button
      ...
      onClick={onToggle}
      className={clsx(
        'aspect-[4/3] rounded-lg border flex flex-col items-center justify-center  select-none outline-none transition-all duration-150 active:scale-90 bg-[var(--color-deep)]',
    ...
    >
      <div className="flex justify-between items-start w-full px-2">
        <FiberManualRecordIcon
          ...
          style={{ fontSize: 10 }}
        />
        <p className={clsx('text-[0.6rem]', active)}>
          {id === "A01" ? "Temp. Ambiente" : id === "A02" ? "mmca" : `${id} - ${fila.substring(0, 3).toUpperCase()}`}
        </p>
      </div>

      <div className="flex w-full justify-center items-baseline gap-1">
        <span className={clsx('text-xl font-bold', habilitado ? 'text-white' : 'text-[var(--color-text-muted)]')}>
